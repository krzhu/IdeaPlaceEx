...
public:
  ...
  explicit SymPair(IndexType cellIdx1, IndexType cellIdx2) {
    this->setCellIdx(cellIdx1, cellIdx2);
  }
  /// @brief set the cell indices. The order stored is always left is lower in
  /// index
  /// @param one cell index
  /// @param another cell index
  ...
  IndexType firstCell() const {
    Assert(_firstCell != INDEX_TYPE_MAX);
    return _firstCell;
  }
  /// @brief get the second cell index
  /// @return the second cell index
  IndexType secondCell() const {
    Assert(_secondCell != INDEX_TYPE_MAX);
    return _secondCell;
  }
...
public:
  ...
  /// @param one cell index
  /// @param another cell index
  void addSymPair(IndexType cellIdx1, IndexType cellIdx2) {
    _symPairs.emplace_back(SymPair(cellIdx1, cellIdx2));
  }
...
private:
  std::vector<IndexType>
      _cells;      ///< The cells belonging to this proximity group
...
private:
  std::vector<IndexType>
      _pinIdxArray; ///< The indices of pins composing the path
