...
enum class IoPinAssignment { UNDEFINED = 0, LEFT = 1, RIGHT = 2 };
...
public:
  ...
  /// @brief get whether need to consider the virtual pin: If not IO net, or if
  /// no vitual pin assigned
  bool isValidVirtualPin() const {
    return (_isIo or _isVdd or _isVss) && _virtualPin.assigned();
  }
  ...
  bool iopinVertical() const {
    return _virtualPin.direction() == Direction2DType::NORTH or
           _virtualPin.direction() == Direction2DType::SOUTH;
  }
  ...
  void setVirtualPin(const VirtualPin &virtualPinLocation) {
    _virtualPin = virtualPinLocation;
  }
  ...
  void setSymNet(IndexType symNet, bool isLeftSym) {
    _symNetIdx = symNet;
    _isLeftSym = isLeftSym;
  }
  ...
  /// @brief get whether this net is assigned to particular location in the
  /// floorplan process
  /// @return true: this is either assigned to left or right. false: no
  bool isAssignedInFpIoPin() const {
    return _fpAssignment != IoPinAssignment::UNDEFINED;
  }
  /// @brief get whether this net is assigned to left
  /// @return true: it is assigned to left. false: no
  bool isLeftAssignedInFpIoPin() const {
    return _fpAssignment == IoPinAssignment::LEFT;
  }
  /// @brief get whether this net is assigned to right
  /// @return true: it is assigned to right. false: no
  bool isRightAssignedInFpIoPin() const {
    return _fpAssignment == IoPinAssignment::RIGHT;
  }
...
private:
  ...
  std::vector<IndexType>
      _pinIdxArray;       ///< The index to the pins belonging to the net
  ...
  IndexType _symNetIdx =
      INDEX_TYPE_MAX; ///< The symmetric pair of this net. If INDEX_TYPE_MAX, it
                      ///< does not have a sym pair
  bool _isLeftSym =
      true; ///< Whethet this should be on the left in sym net pair
  ...
  IoPinAssignment _fpAssignment =
      IoPinAssignment::UNDEFINED; ///< The assignment from the floorplan
  Box<LocType> _externalBBox =
      Box<LocType>(1, 1, -1, -1); ///< The bounding box for the external nets
};
