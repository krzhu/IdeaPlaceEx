...
public:
  ...
  /* Utils                        */
  /*------------------------------*/
  /// @brief get the relative middle point location of the pin with respect to
  /// the cell
...
private:
  ...
  std::vector<IndexType>
      _netIdxArray; ///< The indices of nets connected to this pin
  IndexType _cellIdx = INDEX_TYPE_MAX; ///< The cell this pin belongs to. Each
                                       ///< pin can belong on only one cell
  Box<LocType> _shape =
      Box<LocType>(LOC_TYPE_MAX, LOC_TYPE_MAX, LOC_TYPE_MIN,
                   LOC_TYPE_MIN); ///< The relative location of this pin with
                                  ///< respect to the cell
