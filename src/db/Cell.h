...
public:
  ...
  LocType xCenter() const {
    return _loc.x() + (_cellBBox.xLo() + _cellBBox.xHi()) / 2;
  }
  LocType yCenter() const {
  ...
  void allocateLayers(IndexType numLayers) {
    ...
    _bboxArray.resize(numLayers, Box<LocType>(LOC_TYPE_MAX, LOC_TYPE_MAX,
                                              LOC_TYPE_MIN, LOC_TYPE_MIN));
  }
  /// @brief check whether there is shape in the layer
  /// @return whether the layer has shape
  bool layerHasShape(IndexType layerIdx) const {
    return _bboxArray.at(layerIdx).valid();
  }
  /// @brief union a bounding box in a layer
  /// @param first: the layer index
  /// @param second: the shape bounding box
  void unionBBox(IndexType layerIdx, const Box<LocType> &bbox) {
    _bboxArray.at(layerIdx).unionBox(bbox);
  }
  ...
  /// @brief get the actual bounding box (after being offsetted by the location)
  /// of shapes in a layer
  /// @param first: the layer index
  ...
  void forceExtendToGrid(LocType gridStep) {
    ...
    _cellBBox.setXL(_cellBBox.xl());
    _cellBBox.setXH(_cellBBox.xh());
    _cellBBox.setYL(_cellBBox.yl());
    _cellBBox.setYH(_cellBBox.yh());
  ...
  /*------------------------------*/
  /// @brief calculate the bounding box of the entire cell (union from each
  /// layer)
...
private:
  ...
  std::vector<IndexType>
      _pinIdxArray; ///< The index to the pins belonging to the cell
  std::vector<Box<LocType>> _bboxArray; ///< _shapeArray[layer] = the bounding
                                        ///< box of the shapes in the layer
  Box<LocType> _cellBBox =
      Box<LocType>(LOC_TYPE_MAX, LOC_TYPE_MAX, LOC_TYPE_MIN,
                   LOC_TYPE_MIN); ///< The bounding box of the entire cell
