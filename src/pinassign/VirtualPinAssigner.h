...
public:
  ...
  bool solveFromDB();
  /// @brief solve the pin assignment problem. Export the solution to the
  /// database
  ...
  void setVirtualBoundaryExtension(LocType ex) {
    _virtualBoundaryExtension = ex;
  }
...
private:
  ...
  LocType _virtualBoundaryExtension =
      -1; ///< The extension to placement cell bounding box
  LocType _virtualPinInterval = -1; ///< The interval between virtual pins
  std::map<IndexType, IndexType>
      _leftToRightMap;    // _leftToRightMap[idx of left] = idx of right
