...
void VirtualPinAssigner::reconfigureVirtualPinLocations(
    const Box<LocType> &cellsBBox) {
#ifdef DEBUG_PINASSIGN
  DBG("Ideaplace: pinassgin: %s\n", __FUNCTION__);
#endif
  ...
  if (_db.parameters().hasGridStep()) {
    ...
    LocType targetWidth = std::max(_boundary.xHi() - targetCenter,
                                   targetCenter - _boundary.xLo());
  ...
  for (LocType x = _boundary.xLo() + pinInterval;
       x < _boundary.center().x() - pinInterval / 2; x += pinInterval) {
    continue;
    LocType rightX = 2 * _boundary.center().x() - x;
    if (rightX <= x) {
      ERR("Pin assignment: unexpected pin location x %d center %d right x %d "
          "pinInterval %d \n",
  ...
  for (LocType y = _boundary.yLo() + pinInterval;
       y < _boundary.yHi() - pinInterval; y += pinInterval) {
...
bool VirtualPinAssigner::_networkSimplexPinAssignment(
  ...
  lNodes.reserve(numNets);
  rNodes.reserve(numSites);
  lArcs.reserve(numNets);
  rArcs.reserve(numSites);
  mArcs.reserve(numNets * numSites);
...
bool VirtualPinAssigner::pinAssignment(
    std::function<Point<LocType>(IndexType)> cellLocQueryFunc) {
  ...
  for (IndexType netIdx = 0; netIdx < _db.numNets(); ++netIdx) {
    ...
    curNetBBox.at(netIdx) =
        Box<LocType>(pinOff, pinOff); // Init into an area=0 box
    // The rest pins
  ...
  auto calculateIncreasedHpwl = [&](IndexType netIdx, IndexType pinIdx) {
    ...
    auto difX = std::max(virtualPinLoc.x() - curNetBBox.at(netIdx).xHi(),
                         curNetBBox.at(netIdx).xLo() - virtualPinLoc.x());
    difX = std::max(difX, 0);
    auto difY = std::max(virtualPinLoc.y() - curNetBBox.at(netIdx).yHi(),
                         curNetBBox.at(netIdx).yLo() - virtualPinLoc.y());
    difY = std::max(difY, 0);
    return difX + difY;
  };

  auto calculateShortestManhattanDistance = [&](IndexType netIdx,
                                                IndexType ioPinIdx) {
  ...
  auto calculateShortestManhattanDistancePlusExternalIncreasedHpwl =
      [&](IndexType netIdx, IndexType ioPinIdx) {
        ...
        auto difX = std::max(virtualPinLoc.x() - externalBBox.xHi(),
                             externalBBox.xLo() - virtualPinLoc.x());
        difX = std::max(difX, 0);
        auto difY = std::max(virtualPinLoc.y() - externalBBox.yHi(),
                             externalBBox.yLo() - virtualPinLoc.y());
        difY = std::max(difY, 0);
  ...
  auto directNetToPinCostFunc = [&](IndexType netIdx, IndexType virtualPinIdx) {
    if (not _db.net(netIdx).isAssignedInFpIoPin()) {
      return calculateShortestManhattanDistancePlusExternalIncreasedHpwl(
          netIdx, virtualPinIdx);
    }
    // It has been assigned
    bool isPinLeft =
        _leftToRightMap.find(virtualPinIdx) != _leftToRightMap.end();
    if (isPinLeft) {
  ...
  auto directAssignNetToPinFunc = [&](IndexType netIdx,
                                      IndexType virtualPinIdx) {
    AssertMsg(!_virtualPins[virtualPinIdx].assigned(),
              "Ideaplace: IO pin assignment: unexpected error: pin assignment "
              "conflict \n");
  ...
  auto symPairNetToPinCostFunc = [&](IndexType netIdx, IndexType leftPinIdx) {
    ...
    // if (_db.net(netIdx).isSelfSym())
    //{
    //    return calculateIncreasedHpwl(netIdx, leftPinIdx) +
    //    calculateIncreasedHpwl(netIdx, rightPinIdx);
    //}
    if (_db.net(netIdx).hasSymNet()) {
      auto otherNetIdx = _db.net(netIdx).symNetIdx();
      auto netCost0 =
          calculateShortestManhattanDistancePlusExternalIncreasedHpwl(
              netIdx, leftPinIdx) +
          calculateShortestManhattanDistancePlusExternalIncreasedHpwl(
              otherNetIdx, rightPinIdx);
      auto netCost1 =
          calculateShortestManhattanDistancePlusExternalIncreasedHpwl(
              otherNetIdx, leftPinIdx) +
          calculateShortestManhattanDistancePlusExternalIncreasedHpwl(
              netIdx, rightPinIdx);
  ...
  if (1) {
    if (!_networkSimplexPinAssignment(useSymNet, useLeftPin,
                                      symPairNetToPinCostFunc,
                                      symPairAssignNetToPinFunc)) {
      return false;
    }

    return _networkSimplexPinAssignment(useASymNet, useFreePin,
                                        directNetToPinCostFunc,
                                        directAssignNetToPinFunc);
  }
  return _lpSimplexPinAssignment(
      useSymNet, useLeftPin, useASymNet, useFreePin, symPairNetToPinCostFunc,
      directNetToPinCostFunc, symPairAssignNetToPinFunc,
      directAssignNetToPinFunc);
}
...
bool VirtualPinAssigner::_lpSimplexPinAssignment(
  ...
  std::vector<std::pair<IndexType, IndexType>>
      conflictPins; // .first = sym pair pin. .second = other pin
  for (IndexType netIdx = 0; netIdx < _db.numNets(); ++netIdx) {
  ...
  solver_type solver;
  expr_type obj;
  Vector2D<variable_type> xs(m, ns); // m * ns x decision variables. xs[i, j] ->
                                     // assign symPins[i] to symNets[j]
  Vector2D<variable_type> ys(2 * m,
                             na); // 2*m*na y decision variable. ys[i,
                                  // j]->assign  otherPins[i] tootherNets[j]
  ...
  for (IndexType x = 0; x < 2 * m; ++x) {
    for (IndexType y = 0; y < na; ++y) {
      obj += ys.at(x, y) *
             (otherNetToPinCostFunc(otherNets[y], otherPins[x]) + 0.0);
    }
  }
  ...
  AssertMsg(!failed,
            "Ideaplace:: virtual pin assignment:: ILP:: floating results \n");

  auto end = std::chrono::high_resolution_clock::now();
  std::cout << " io pin assignment time "
