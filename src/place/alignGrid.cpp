...
void GridAligner::align(LocType stepSize) {
  ...
  for (IndexType cellIdx = 0; cellIdx < _db.numCells(); ++cellIdx) {
    ...
    INF("IDEAPLACE::%s cell %d %s center %d %d\n ", __FUNCTION__, cellIdx,
        cell.name().c_str(), cell.xCenter(), cell.yCenter());
  }
  for (IndexType cellIdx = 0; cellIdx < _db.numCells(); ++cellIdx) {
    const auto &cell = _db.cell(cellIdx);
...
LocType GridAligner::findCurrentSymAxis() {
  ...
  for (IndexType symGrpIdx = 0; symGrpIdx < _db.numSymGroups(); ++symGrpIdx) {
    ...
    for (IndexType symPairIdx = 0; symPairIdx < symGrp.numSymPairs();
      ...
      symAxis = (_db.cell(symPair.firstCell()).xLo() +
                 _db.cell(symPair.secondCell()).xHi()) /
                2;
      foundSym = true;
      goto theEnd;
    }
  ...
  if (!foundSym) {
    ...
    xCenter = xCenter + ceilDif(xCenter, _stepSize) +
              _stepSize / 2; // Ensure its on the correct grid
    return xCenter;
  }
theEnd:
  return symAxis;
...
void GridAligner::adjustSymPair(const SymPair &symPair, LocType symAxis) {
  ...
  if (_db.cell(symPair.firstCell()).xLoc() <
      _db.cell(symPair.secondCell()).xLoc()) {
  ...
  _db.cell(leftCellIdx)
      .setXLoc(_db.cell(leftCellIdx).xLoc() -
               floorDif(_db.cell(leftCellIdx).xLo(), _stepSize));
...
struct CellIdxNode {
  CellIdxNode(IndexType cellIdx_, std::vector<char> *fixed_,
              std::vector<char> *hasSym_, std::vector<LocType> *dis2SymAxis_)
      : cellIdx(cellIdx_), fixed(fixed_), hasSym(hasSym_),
        dis2SymAxis(dis2SymAxis_) {}
  IndexType cellIdx;
...
void GridAligner::bettherThanNaiveAlign() {
  ...
  auto symDif =
      floorDif(static_cast<IntType>(symAxis - 0.5 * _stepSize), _stepSize);
  ...
  // fixed all syms
  using HeapType =
      boost::heap::fibonacci_heap<CellIdxNode,
                                  boost::heap::compare<compareCellIdxNode>>;
  ...
  for (IndexType cellIdx = 0; cellIdx < _db.numCells(); ++cellIdx) {
    heapHandles.emplace_back(cellNodeHeap.push(
        CellIdxNode(cellIdx, &xDecided, &hasSym, &dis2SymAxis)));
  }
  ...
  while (!cellNodeHeap.empty()) {
    ...
    for (IndexType target : edgeLUT.at(cellIdx)) {
      ...
      if (inHeap.at(target)) {
        cellNodeHeap.update(
            heapHandles.at(target),
            CellIdxNode(target, &xDecided, &hasSym, &dis2SymAxis));
      } else {
        inHeap.at(target) = true;
        heapHandles.at(target) = cellNodeHeap.push(
            CellIdxNode(target, &xDecided, &hasSym, &dis2SymAxis));
      }
      if (_db.cell(target).hasSymPair() && checkSym) {
        ...
        if (inHeap.at(symNetIdx)) {
          cellNodeHeap.update(
              heapHandles.at(symNetIdx),
              CellIdxNode(symNetIdx, &xDecided, &hasSym, &dis2SymAxis));
        } else {
          inHeap.at(symNetIdx) = true;
          heapHandles.at(symNetIdx) = cellNodeHeap.push(
              CellIdxNode(symNetIdx, &xDecided, &hasSym, &dis2SymAxis));
        }
      }
    ...
    for (IndexType source : edgeLUTReverse.at(cellIdx)) {
      ...
      if (inHeap.at(source)) {
        cellNodeHeap.update(
            heapHandles.at(source),
            CellIdxNode(source, &xDecided, &hasSym, &dis2SymAxis));
      } else {
        inHeap.at(source) = true;
        heapHandles.at(source) = cellNodeHeap.push(
            CellIdxNode(source, &xDecided, &hasSym, &dis2SymAxis));
      }
