...
void LpLegalizeSolver::exportSolution() {
  for (IndexType cellIdx = 0; cellIdx < _db.numCells(); ++cellIdx) {
    ...
    if (_isHor) {
      _db.cell(cellIdx).setXLo(::klib::autoRound<LocType>(var) +
                               _db.parameters().layoutOffset());
    } else {
      _db.cell(cellIdx).setYLo(::klib::autoRound<LocType>(var) +
                               _db.parameters().layoutOffset());
    }
  }
}
  ...
  if (_optHpwl == 1) {
    ...
    for (IndexType netIdx = 0; netIdx < _db.numNets(); ++netIdx) {
      ...
      if (_db.net(netIdx).numPinIdx() == 1 &&
          !_db.net(netIdx).isValidVirtualPin()) {
        continue;
      }
...
void LpLegalizeSolver::addSymObj() {
  if (_isHor) {
    for (IndexType symGrpIdx = 0; symGrpIdx < _symRexLeft.size(); ++symGrpIdx) {
      _obj +=
          _largeNum * (_symRexRight.at(symGrpIdx) - _symRexLeft.at(symGrpIdx));
    }
  } else {
    ...
    if (_optArea == 0) {
      if (_isHor) {
        ...
        DBG("Add boundary constraint: x_%d <= %f - %d \n", i, _wStar,
            _db.cell(i).cellBBox().xLen());
#endif
        lp_trait::addConstr(
            _solver, _locs.at(i) <= _wStar + _db.parameters().layoutOffset() -
                                        _db.cell(i).cellBBox().xLen());
      } else {
#ifdef DEBUG_LEGALIZE
        DBG("Add boundary constraint: y_%d <= %f - %d \n", i, _wStar,
            _db.cell(i).cellBBox().yLen());
#endif
        lp_trait::addConstr(
            _solver, _locs.at(i) <= _wStar + _db.parameters().layoutOffset() -
                                        _db.cell(i).cellBBox().yLen());
      ...
      lp_trait::addConstr(_solver,
                          _locs.at(i) >= +_db.parameters().layoutOffset());
    } else // if (_optArea == 0)
    {
      if (_isHor) {
        // 0 <= x_i <= W - w_i
        lp_trait::addConstr(_solver, _locs.at(i) - _dim <=
                                         -_db.cell(i).cellBBox().xLen());
      } else {
        lp_trait::addConstr(_solver, _locs.at(i) - _dim <=
                                         -_db.cell(i).cellBBox().yLen());
...
void LpLegalizeSolver::addTopologyConstraints() {
  for (auto &edge : _constrains.edges()) {
    ...
    LocType spacing;
    // Force cell1 to be lower/left to the cell2. Therefore only using
    // spacingBox.xLo() and .yLo()
    if (_isHor) {
    ...
    // Add the constraint
    // x_i + w_i + spacing <= x_j
    lp_trait::addConstr(_solver, _locs.at(sourceIdx) - _locs.at(targetIdx) <=
                                     -cellDim - spacing);
#ifdef DEBUG_LEGALIZE
    DBG("Add spacing constrain: from %d to %d, <= -celldim %d - spacing %d = "
        "%d \n",
  ...
  if (_isHor) {
    for (IndexType symGroupIdx = 0; symGroupIdx < _db.numSymGroups();
      ...
      for (IndexType symPairIdx = 0; symPairIdx < symGroup.numSymPairs();
        ...
        auto width_ = _db.cell(symPair.firstCell())
                          .cellBBox()
                          .xLen(); // Two cells are equal in width <- assumption
    ...
    for (IndexType symGroupIdx = 0; symGroupIdx < _db.numSymGroups();
      ...
      for (IndexType symPairIdx = 0; symPairIdx < symGroup.numSymPairs();
        ...
        // y_b - y_t <= 0
        lp_trait::addConstr(_solver,
                            _locs.at(bCellIdx) - _locs.at(tCellIdx) <= 0.0);
      }
    }
  }
}
    ...
    for (IndexType symGrpIdx = 0; symGrpIdx < _db.numSymGroups(); ++symGrpIdx) {
      ...
      for (IndexType symPairIdx = 0; symPairIdx < symGrp.numSymPairs();
        ...
        DBG("Add sym constraint. \n symGrp %d, cell %d %d \n width %d \n",
            symGrpIdx, symPair.firstCell(), symPair.secondCell(),
            _db.cell(symPair.firstCell()).cellBBox().xLen());
#endif
        lp_trait::addConstr(
            _solver,
            _locs.at(symPair.firstCell()) + _locs.at(symPair.secondCell()) -
                    2 * (*symVar) ==
                -_db.cell(symPair.firstCell())
                     .cellBBox()
                     .xLen()); // Two cells are equal in width <- assumption
        AssertMsg(_db.cell(symPair.firstCell()).cellBBox().xLen() ==
                      _db.cell(symPair.secondCell()).cellBBox().xLen(),
                  "cell %s and cell %s \n",
                  _db.cell(symPair.firstCell()).name().c_str(),
                  _db.cell(symPair.secondCell()).name().c_str());
      }
    ...
    for (IndexType symGroupIdx = 0; symGroupIdx < _db.numSymGroups();
      ...
      for (IndexType symPairIdx = 0; symPairIdx < symGroup.numSymPairs();
        ...
        DBG("Add y sym constraint. \n symGrp %d, cell %d %d  \n", symGroupIdx,
            symPair.firstCell(), symPair.secondCell());
#endif
      }
    }
  }
    ...
    for (IndexType netIdx = 0; netIdx < _db.numNets(); ++netIdx) {
      ...
      for (IndexType pinIdxInNet = 0; pinIdxInNet < net.numPinIdx();
        ...
        if (_isHor) {
          ...
          lp_trait::addConstr(_solver,
                              _wlL.at(netIdx) - _locs.at(pin.cellIdx()) <= loc);
          // wl_r >= _loc + pin_offset for all pins in the net
          lp_trait::addConstr(_solver,
                              _wlR.at(netIdx) - _locs.at(pin.cellIdx()) >= loc);
        } else {
      ...
      if (net.isValidVirtualPin()) {
        if (_isHor) {
          ...
          // wl_l <= _loc + pin_offset for all pins in the net
          lp_trait::addConstr(_solver, _wlL.at(netIdx) <= std::max(loc, 0.0));
          // wl_r >= _loc + pin_offset for all pins in the net
          lp_trait::addConstr(_solver, _wlR.at(netIdx) >= loc);
        } else {
...
void LpLegalizeSolver::addCurrentFlowConstraints() {
  ...
  if (not _useCurrentFlowConstraint) {
    return;
  }
  ...
  for (IndexType pathIdx = 0; pathIdx < pathMgr.vvSegList().size(); ++pathIdx) {
    ...
    for (const auto &seg : path) {
      ...
      lp_trait::addConstr(_solver, _locs.at(mCellIdx) - _locs.at(sCellIdx) <=
                                       sPinOffset.y() - midPinOffsetA.y());
      lp_trait::addConstr(_solver, _locs.at(tCellIdx) - _locs.at(mCellIdx) <=
                                       midPinOffsetB.y() - tPinOffset.y());
    }
  }
}
