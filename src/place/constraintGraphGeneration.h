 ...
 * @brief The reimplement of the sweep line and transitive reduction algorithm
 * on generating the constraint graph for legalization
 * @author Keren Zhu
 * @date 01/26/2020
 */
...
/// Basically, when finding horizontal edges. a sweep line is visited all cell
/// in vertical direction. The events determine when to add or removes the
/// devices as the sweep line cross the cell edge.
class Event {
public:
  ...
  /// When two coordinates are the same, the high events must before the low
  /// events. NOTE: what if there are multiple same coodinates?
  bool operator<(const Event &rhs) const {
    if (_loc == rhs.loc()) {
      if (_isLow == rhs.isLow()) {
        return _cellIdx < rhs.cellIdx();
      }
      return !_isLow; // if lhs is low, lhs > rhs. if lhs is high (!_isLow), lhs
                      // < rhs.
    }
    return _loc < rhs.loc();
  }
...
private:
  IndexType _cellIdx =
      INDEX_TYPE_MAX; ///< The index of cell this event belonging to
  LocType _loc =
      LOC_TYPE_MIN;   ///< The coordinate of the event. If finding horzional
                      ///< edges, the loc is the lower/higher y edge.
...
public:
  ...
  IndexType cellIdx() const { return _cellIdx; }
  LocType loc() const { return _loc; }
  /// @brief comparison operator. \lambda relation in the original paper x_i >
  /// x_j or (xi = xj and i > j). Here take itsopposite
...
public:
  explicit SweeplineConstraintGraphGenerator(Database &db, Constraints &hC,
                                             Constraints &vC)
...
private:
  ...
  /// @param first: the referece to a vector of events. The vector will be
  /// cleared first.
  /// @param second: true: generating horizontal edges. false: generating
  /// vertical edges
  ...
  /// @param first: A reference to the vector of cell coordinates. The vector
  /// will be cleared in the beginning
  ...
  // void originalConstraintGeneration(Constraints &cs, std::vector<Event>
  // &events, std::vector<CellCoord> &cellCoords);
  void originalDelete(const CellCoord &cellCoord, CellCoordTree &dTree,
                      std::vector<IntType> &cand, Constraints &cs);
  void originalConstraintGeneration(Constraints &cs, std::vector<Event> &events,
