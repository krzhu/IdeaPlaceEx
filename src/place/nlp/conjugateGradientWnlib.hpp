...
namespace _wnlib {
template <typename nlp_numerical_type> struct wnlibWrapper {
  wnlibWrapper(std::function<nlp_numerical_type(void)> evaObj,
               std::function<void(void)> evaGrad, nlp_numerical_type *pl,
               nlp_numerical_type *grad, IndexType numVariables)
...
struct conjugate_gradient_wnlib {
  template <typename nlp_type> void optimize(nlp_type &n) {
    auto evaObj = [&]() {
      n._wrapObjAllTask.run();
      return n._obj;
    };
    auto evaGrad = [&]() { n._wrapCalcGradTask.run(); };
    _wnlib::wnlibWrapper<typename nlp_type::nlp_numerical_type> wrapper(
        evaObj, evaGrad, n._pl.data(), n._grad.data(), n._numVariables);
    _wnlib::wrapperwrapperPtr = &wrapper;
    wrapper.optimize();
  }
};
} // namespace first_order
template <> struct optm_trait<first_order::conjugate_gradient_wnlib> {
