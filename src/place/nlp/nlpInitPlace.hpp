...
/* Initial placement */
template <typename T> struct init_place_trait {
...
struct init_random_placement_with_number_of_cells_uniform_distribution {
  static constexpr RealType range =
      0.8; ///< Distrubute the location into range ratio of the boundary
};

template <>
struct init_place_trait<
    init_random_placement_with_number_of_cells_uniform_distribution> {
  typedef init_random_placement_with_number_of_cells_uniform_distribution T;
  template <typename NlpType> static T construct(NlpType &) { return T(); }
  template <typename NlpType> static void initPlace(T &, NlpType &nlp) {
...
struct init_random_placement_with_normal_distribution_near_center {
  static constexpr RealType randomInitPlaceStddev =
      0.01; ///< The standard deviation of the initial random placement in terms
            ///< of the width/height of the boundary
};

template <>
struct init_place_trait<
    init_random_placement_with_normal_distribution_near_center> {
  ...
  template <typename NlpType> static void initPlace(T &, NlpType &nlp) {
    ...
    RealType stddev =
        init_random_placement_with_normal_distribution_near_center::
            randomInitPlaceStddev *
        std::min(nlp._boundary.xLen(), nlp._boundary.yLen());
    ...
    for (IndexType cellIdx = 0; cellIdx < numCells; ++cellIdx) {
      const coord_type xOffset =
          nlp._db.cell(cellIdx).cellBBox().xLen() * nlp._scale;
      const coord_type yOffset =
          nlp._db.cell(cellIdx).cellBBox().yLen() * nlp._scale;
      const auto xIdx = nlp.plIdx(cellIdx, Orient2DType::HORIZONTAL);
      const auto yIdx = nlp.plIdx(cellIdx, Orient2DType::VERTICAL);
