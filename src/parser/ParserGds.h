...
public:
  ...
  /// @param second: the cell index. If given INDEX_TYPE_MAX or ungiven, search
  /// the cell by name
  /// @return if the parsing is successful
  bool parseCellGds(const std::string &filename,
...
inline bool parseAllGdsFiles(Database &db,
                             const std::vector<std::string> &gdsFiles) {
...
inline IndexType layerIdx(IntType gdsLayer,
                          std::unordered_map<IndexType, IndexType> &map) {
  /// Search in the map
...
inline void extractShape(std::unordered_map<IndexType, IndexType> &map,
                         std::vector<ParserCellGds::PolygonLayer> &polygons,
...
inline void extractShape(std::unordered_map<IndexType, IndexType> &map,
                         std::vector<ParserCellGds::PolygonLayer> &polygons,
                         ::GdsParser::GdsRecords::EnumType type,
                         ::GdsParser::GdsDB::GdsRectangle *object) {
  DBG("Rect \n");
  AssertMsg(0, "%s: Rectangle type not support yet \n", __FUNCTION__);
  // DBG("Gds rectangle layer %d, (%d, %d ), (%d, %d) \n", object->layer(),
  // gtl::xl(object), gtl::yl(object), gtl::xh(object), gtl::yh(object));
}
...
inline void extractShape(std::unordered_map<IndexType, IndexType> &map,
                         std::vector<ParserCellGds::PolygonLayer> &polygons,
                         ::GdsParser::GdsRecords::EnumType type,
                         ::GdsParser::GdsDB::GdsPolygon *object) {
...
inline void extractShape(std::unordered_map<IndexType, IndexType> &map,
                         std::vector<ParserCellGds::PolygonLayer> &polygons,
                         ::GdsParser::GdsRecords::EnumType type,
                         ::GdsParser::GdsDB::GdsPath *object) {
...
struct ExtractShapeLayerAction {
  /// @param first: the map between the gds layer indices to the router layers.
  /// Only care about the mapping within this map
  /// @param second: a reference to a vector of polygons, saved the read
  /// polygons in this vector
  ExtractShapeLayerAction(std::unordered_map<IndexType, IndexType> &map,
                          std::vector<ParserCellGds::PolygonLayer> &polygons)
      : _layerIdxMap(map), _polygons(polygons) {}
  template <typename ObjectType>
  void operator()(::GdsParser::GdsRecords::EnumType type, ObjectType *object) {
    ExtractShapeLayerActionDetails::extractShape(_layerIdxMap, _polygons, type,
                                                 object);
  }

  /// @return a message of action for debug
  std::string message() const { return "ExtractShapeLayerAction"; }

  std::unordered_map<IndexType, IndexType>
      &_layerIdxMap; ///< A map from gds layer to IDEAPLACE layer. _layerIdxMap[
  std::vector<ParserCellGds::PolygonLayer>
      &_polygons; ///< The polygons read from the gds
};
