...
bool ParserSymNet::processNamePair() {
  ...
  for (const auto &pair : _pairs) {
    if (netNameMap.find(pair.first) != netNameMap.end() &&
        netNameMap.find(pair.second) != netNameMap.end()) {
      ...
      INF("ParserSymNet:: left net %s %d\n, right net %s %d\n",
          _db.net(netIdx1).name().c_str(), netIdx1,
          _db.net(netIdx2).name().c_str(), netIdx2);
    } else {
      ERR("Symnet parser: symmetry net pair: %s %s are not inside the netlist! "
          "Ignored. \n",
  ...
  for (const auto &self : _selfs) {
    if (netNameMap.find(self) != netNameMap.end()) {
      ...
      ERR("Symnet parser: self symmetry net: %s  are not inside the netlist! "
          "Ignored. \n",
          self.c_str());
    }
  }
