...
/**
 * @brief if your point class setting is different from that in the default
 * point_traits, please create a specialization
 *
 * The specialization should be in the same scope as the original template
 * struct
 */
template <typename T> struct point_traits<PROJECT_NAMESPACE::Point<T>> {
  /// point type
...
/**
 * @brief if your rectangle class setting is different from that in the default
 * point_traits, please create a specialization
 */
template <typename T> struct rectangle_traits<PROJECT_NAMESPACE::Box<T>> {
  /// rectangle type
   ...
   */
  static void set(rectangle_type &rect, direction_2d dir,
                  coordinate_type value) {
    switch (dir) {
    case LEFT:
      rect.setXLo(value);
   ...
   */
  static rectangle_type construct(coordinate_type xl, coordinate_type yl,
