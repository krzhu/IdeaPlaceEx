...
public:
  ...
  void shiftXY(const T x, const T y) {
    _d[0] += x;
    _d[1] += y;
  }
  ...
  static bool
  bOnSegment(const Point &p, const Point &q,
             const Point &r); // checks if point q lies on line segment 'pr'
  static int orientation(
      const Point &p, const Point &q,
      const Point &r); // 0: p,q,r colinear; 1: clockwise; -1:counterclockwise
  ...
  bool operator==(const Point &p) const {
    return _d[0] == p._d[0] && _d[1] == p._d[1];
  }
  ...
  Point operator+(const Point &p) const {
    return Point(_d[0] + p._d[0], _d[1] + p._d[1]);
  }
  Point operator-(const Point &p) const {
    return Point(_d[0] - p._d[0], _d[1] - p._d[1]);
  }
  long long operator^(const Point &p) const {
    return (long long)_d[0] * p._d[1] - (long long)p._d[0] * _d[1];
  }
  bool operator<=(const Point &p) const {
    return _d[0] <= p._d[0] && _d[1] <= p._d[1];
  }
  ...
  template <typename U> Point<T> &operator*=(U rhs) {
    _d[0] *= rhs;
    _d[1] *= rhs;
    return *this;
  }
  ...
  struct CompareX {
    bool operator()(const Point &p1, const Point &p2) {
      return p1._d[0] != p2._d[0] ? p1._d[0] < p2._d[0] : p1._d[1] < p2._d[1];
    }
  };
  struct CompareY {
    bool operator()(const Point &p1, const Point &p2) {
      return p1._d[1] != p2._d[1] ? p1._d[1] < p2._d[1] : p1._d[0] < p2._d[0];
    }
  };
...
template <typename T> void Point<T>::rotate180(const T x, const T y) {
  _d[0] = x + (x - _d[0]);
  _d[1] = y + (y - _d[1]);
}

template <typename T> void Point<T>::flipX(const T x) { _d[0] = 2 * x - _d[0]; }

template <typename T> void Point<T>::flipY(const T y) { _d[1] = 2 * y - _d[1]; }
...
int Point<T>::orientation(const Point &p, const Point &q, const Point &r) {
  T val = (q._d[1] - p._d[1]) * (r._d[0] - q._d[0]) -
          (q._d[0] - p._d[0]) * (r._d[1] - q._d[1]);
  if (val == 0)
    return 0;                // colinear
...
bool Point<T>::bOnSegment(const Point &p, const Point &q, const Point &r) {
  if (q._d[0] <= std::max(p._d[0], r._d[0]) and
      q._d[0] >= std::min(p._d[0], r._d[0]) and
      q._d[1] <= std::max(p._d[1], r._d[1]) and
      q._d[1] >= std::min(p._d[1], r._d[1]))
    return true;
...
//    std::size_t operator() (const PROJECT_NAMESPACE::Point<CoordType>& p)
//    const {
...
namespace polygon {
template <typename CoordType>
struct geometry_concept<PROJECT_NAMESPACE::Point<CoordType>> {
...
struct point_traits<PROJECT_NAMESPACE::Point<CoordType>> {
  ...
  get(const PROJECT_NAMESPACE::Point<CoordType> &point, orientation_2d orient) {
...
struct point_mutable_traits<PROJECT_NAMESPACE::Point<CoordType>> {
  typedef CoordType coordinate_type;
  static void inline set(PROJECT_NAMESPACE::Point<CoordType> &point,
                         orientation_2d orient, CoordType value) {
    if (orient == HORIZONTAL) {
      point.setX(value);
    } else {
      point.setY(value);
  ...
  static inline PROJECT_NAMESPACE::Point<CoordType>
  construct(CoordType x_value, CoordType y_value) {
...
struct interval_traits<PROJECT_NAMESPACE::Point<CoordType>> {
  typedef CoordType coordinate_type;
  static inline coordinate_type
  get(const PROJECT_NAMESPACE::Point<CoordType> &interval, direction_1d dir) {
    if (dir == LOW) {
      return interval.x();
    } else {
      return interval.y();
...
struct interval_mutable_traits<PROJECT_NAMESPACE::Point<CoordType>> {
  typedef CoordType coordinate_type;
  static inline void set(PROJECT_NAMESPACE::Point<CoordType> &interval,
...
template <typename CoordType> struct tag<PROJECT_NAMESPACE::Point<CoordType>> {
...
template <typename CoordType>
struct dimension<PROJECT_NAMESPACE::Point<CoordType>> : boost::mpl::int_<2> {};
...
struct access<PROJECT_NAMESPACE::Point<CoordType>, 0> {
  ...
  static inline void set(PROJECT_NAMESPACE::Point<CoordType> &p,
