...
class Ring
    : public std::vector<Point<CoordType>, std::allocator<Point<CoordType>>> {
  typedef std::vector<Point<CoordType>, std::allocator<Point<CoordType>>>
      baseType;
...
public:
  ...
  inline Ring(Iterator begin, Iterator end) : baseType(begin, end) {}
...
template <typename CoordType> struct tag<PROJECT_NAMESPACE::Ring<CoordType>> {
...
template <typename CoordType> class Polygon {
public:
  ...
  void setOuter(const RingType &v) { _outer = v; }

  bool operator==(const Polygon &p) const {
    return _outer == p._outer and _inners == p._inners;
  }
...
struct interior_const_type<PROJECT_NAMESPACE::Polygon<CoordType>> {
  typedef
      typename PROJECT_NAMESPACE::Polygon<CoordType>::InnerContainerType const
          &type;
};
...
struct interior_mutable_type<PROJECT_NAMESPACE::Polygon<CoordType>> {
  typedef
      typename PROJECT_NAMESPACE::Polygon<CoordType>::InnerContainerType &type;
};
...
struct exterior_ring<PROJECT_NAMESPACE::Polygon<CoordType>> {
  ...
  static inline typename polygon_type::RingType const &
  get(const polygon_type &p) {
    return p.outer();
  }
};
...
struct interior_rings<PROJECT_NAMESPACE::Polygon<CoordType>> {
  ...
  typedef PROJECT_NAMESPACE::Polygon<CoordType> polygon_type;
  static inline typename polygon_type::InnerContainerType &
  get(polygon_type &p) {
    return p.inners();
  }

  static inline typename polygon_type::InnerContainerType const &
  get(const polygon_type &p) {
    return p.inners();
  }
};
...
// One of the key difference in data between 90 and polygon is that polygon90 is
// using compact representation For example, if compact coordinates are x1, y1,
// e 2, y2, its points in polygon is (x1, y1) (x2, y1) (x2 y2). In other words,
// also we can register Polygon as Polygon90, its efficiency would be lower than
// using native compact data structure.
//
...
struct polygon_90_traits<PROJECT_NAMESPACE::Polygon<CoordType>> {
  ...
  typedef
      typename PROJECT_NAMESPACE::Polygon<CoordType>::RingType::const_iterator
          pointer_iterator_type;
  typedef typename PROJECT_NAMESPACE::Polygon<CoordType>::PointType PointType;
  typedef iterator_points_to_compact<pointer_iterator_type, PointType>
      compact_iterator_type;

  static inline compact_iterator_type
  begin_compact(const PROJECT_NAMESPACE::Polygon<CoordType> &t) {
    return compact_iterator_type(t.outer().begin(), t.outer().end());
  }
  static inline compact_iterator_type
  end_compact(const PROJECT_NAMESPACE::Polygon<CoordType> &t) {
    return compact_iterator_type(t.outer().end(), t.outer().end());
  }
  static inline unsigned int
  size(const PROJECT_NAMESPACE::Polygon<CoordType> &t) {
    return t.outer().size();
  }
  static inline winding_direction
  winding(const PROJECT_NAMESPACE::Polygon<CoordType> &t) {
...
struct polygon_90_mutable_traits<PROJECT_NAMESPACE::Polygon<CoordType>> {
  ...
  static inline PROJECT_NAMESPACE::Polygon<CoordType> &
  set_compact(PROJECT_NAMESPACE::Polygon<CoordType> &t, iT input_begin,
              iT input_end) {
...
struct polygon_traits<PROJECT_NAMESPACE::Polygon<CoordType>> {
  ...
  begin_points(const PROJECT_NAMESPACE::Polygon<CoordType> &p) {
    return p.outer().begin();
  }

  static inline iterator_type
  end_points(const PROJECT_NAMESPACE::Polygon<CoordType> &p) {
    return p.outer().end();
  }

  static inline unsigned int
  size(const PROJECT_NAMESPACE::Polygon<CoordType> &p) {
    return p.outer().size();
  }

  static inline winding_direction
  winding(const PROJECT_NAMESPACE::Polygon<CoordType> &p) {
    return boost::polygon::winding_direction::clockwise_winding; // clockwise
  }
};
...
struct polygon_mutable_traits<PROJECT_NAMESPACE::Polygon<CoordType>> {
  template <typename iT>
  static inline PROJECT_NAMESPACE::Polygon<CoordType> &
  set_points(PROJECT_NAMESPACE::Polygon<CoordType> &t, iT input_begin,
             iT input_end) {
    t.outer().clear();
...
template <typename CoordType> using PolygonSet = std::vector<CoordType>;
...
struct polygon_set_traits<PROJECT_NAMESPACE::PolygonSet<CoordType>> {
  typedef CoordType coordinate_type;
  typedef typename PROJECT_NAMESPACE::PolygonSet<CoordType>::const_iterator
      iterator_type;
  typedef PROJECT_NAMESPACE::PolygonSet<CoordType> operator_arg_type;

  static inline iterator_type
  begin(const PROJECT_NAMESPACE::PolygonSet<CoordType> &t) {
    return view_as<polygon_90_concept>(t).begin();
  }

  static inline iterator_type
  end(const PROJECT_NAMESPACE::PolygonSet<CoordType> &t) {
    return view_as<polygon_90_concept>(t).end();
  }

  static inline bool clean(const PROJECT_NAMESPACE::PolygonSet<CoordType> &t) {
    return false;
  }
  static inline bool sorted(const PROJECT_NAMESPACE::PolygonSet<CoordType> &t) {
    return false;
  }
};
#if 0
