...
/// the returning type.
template <typename T> inline T autoRound(RealType rValue) {
  return static_cast<T>(std::round(rValue));
}

template <> inline float autoRound(RealType rValue) { return rValue; }

template <> inline double autoRound(RealType rValue) { return rValue; }
