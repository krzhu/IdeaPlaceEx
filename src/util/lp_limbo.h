/**
 * @file lp_limbo.h
 * @brief The linear programming implementation using limbo::LPSolveApi and
 * limbo::GurobiLinearApi
 * @author Keren Zhu
 * @date 03/10/2020
 */
...
template <typename limbo_lp_api> struct _limbo_lp_api_trait {
...
template <typename limbo_lp_api_type> class _limbo_lp_solver {
  friend linear_programming_trait<_limbo_lp_solver<limbo_lp_api_type>>;

public:
  typedef
      typename _limbo_lp_api_trait<limbo_lp_api_type>::value_type value_type;
  typedef typename limbo::solvers::LinearModel<value_type, value_type>
      model_type;
  ...
  typedef
      typename _limbo_lp_api_trait<limbo_lp_api_type>::param_type param_type;
  typedef typename _limbo_lp_api_trait<limbo_lp_api_type>::limbo_solver_type
      limbo_solver_type;
...
template <typename value_type> struct _limbo_lp_api_lpsove {};
...
struct _limbo_lp_api_trait<_limbo_lp_api_lpsove<coe_value_type>> {
  ...
  typedef limbo::solvers::LPSolveLinearApi<value_type, value_type>
      limbo_solver_type;
...
struct linear_programming_trait<_limbo_lp_solver<limbo_lp_api_type>> {
  ...
  static variable_type addVar(solver_type &solver) {
    return solver._model.addVariable(0, 1e9, limbo::solvers::CONTINUOUS,
  ...
  static status_type status(solver_type &solver) { return solver._status; }
  ...
  static value_type evaluateExpr(const solver_type &solver,
    ...
    return solver._model.evaluateExpression(expr,
                                            solver._model.variableSolutions());
  }
  ...
  static void setNumThreads(solver_type &solver, std::uint32_t numThreads) {
    _limbo_lp_api_trait<limbo_lp_api_type>::setNumThreads(solver._params,
                                                          numThreads);
  }
};
...
namespace _lp {
#ifndef LP_NOT_USE_LPSOLVE
typedef _lp::_limbo_lp_solver<_lp::_limbo_lp_api_lpsove<double>>
    LimboLpsolve; ///< The lpsolve using limbo api. This one need to be
                  ///< constructed
typedef _lp::linear_programming_trait<LimboLpsolve>
    LimboLpsolveTrait; ///< The lpsolve using limbo api. Don't construct it. Use
                       ///< its static methods as interface
#endif
#ifndef LP_NOT_USE_GUROBI
typedef _lp::_limbo_lp_solver<_lp::_limbo_lp_api_gurobi<double>>
    LimboLpGurobi; ///< The Gurobi using limbo api. This one need to be
                   ///< constructed
typedef _lp::linear_programming_trait<LimboLpGurobi>
    LimboLpGurobiTrait; ///< The Gurobi using limbo api. Don't construct it. Use
                        ///< its static methods as interface
#endif
} // namespace _lp
