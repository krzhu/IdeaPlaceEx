...
public:
  ...
  enum class InitListType : Byte { XMajor, yMajor };

public:
  explicit Vector2D() = default;
  explicit Vector2D(IndexType xSize, IndexType ySize)
      : _vec(xSize * ySize), _xSize(xSize), _ySize(ySize) {}
  explicit Vector2D(IndexType xSize, IndexType ySize, const T &v)
      : _vec(xSize * ySize, v), _xSize(xSize), _ySize(ySize) {}
  explicit Vector2D(IndexType xSize, IndexType ySize, InitListType t,
  ...
  void resize(IndexType xSize, IndexType ySize) {
    _vec.resize(xSize * ySize);
    _xSize = xSize;
    _ySize = ySize;
  }
  void resize(IndexType xSize, IndexType ySize, const T &v) {
    _vec.resize(xSize * ySize, v);
    _xSize = xSize;
    _ySize = ySize;
  }
  ...
  const T &at(IndexType x, IndexType y) const {
    return this->atIdx(xyToIndex(x, y));
  }
  ...
  IndexType xyToIndex(Point<IndexType> xy) const {
    return xyToIndex(xy.x(), xy.y());
  }
  IndexType idxToX(IndexType idx) const;
  IndexType idxToY(IndexType idx) const;
  Point<IndexType> idxToXY(IndexType idx) const {
    return Point<IndexType>(idxToX(idx), idxToY(idx));
  }
...
template <typename T>
inline Vector2D<T>::Vector2D(IndexType xSize, IndexType ySize, InitListType t,
                             std::initializer_list<T> l)
    : _vec(xSize * ySize), _xSize(xSize), _ySize(ySize) {
  Assert(l.size == size());
