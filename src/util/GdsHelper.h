...
/// @brief SREF type
template <>
inline void getSName(std::string &name,
                     ::GdsParser::GdsDB::GdsCellReference *object) {
  name = object->refCell();
}
} // namespace GetSRefNameActionDetails
...
struct GetSRefNameAction {
  ...
  std::string message() const { return "GetSRefNameAction"; }
...
inline std::string topCell(const ::GdsParser::GdsDB::GdsDB &db) {
  ...
  for (auto &cell : db.cells()) {
    for (auto obj : cell.objects()) {
      std::string name = "";
      ::GdsParser::GdsDB::GdsObjectHelpers()(obj.first, obj.second,
                                             GetSRefNameAction(name));
      if (name != "") {
        nameFound[name] = true;
      }
    }
