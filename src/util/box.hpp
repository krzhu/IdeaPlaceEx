...
public:
  ...
  friend std::ostream& operator << (std::ostream& os, const Box& r) {
    ...
    Box<T>          offsetBox(const Point<T> &origin) const { return Box<T>(xLo() + origin.x(), yLo() + origin.y(), xHi() + origin.x(), yHi() + origin.y()); }
    void            offsetBy(const Point<T> &origin) { setXLo(xLo() + origin.x()); setYLo(yLo() + origin.y()); setXHi(xHi() + origin.x()); setYHi(yHi() + origin.y()); }
  //Debugging
...
template<typename T>
void Box<T>::multi_diff(const std::list<std::uint32_t>& ord, const std::vector<Box<T>>& vBox, std::vector<Box<T>>& result) const {
    std::vector<T> hor = { _bl.y(), _tr.y() };
    std::vector<T> ver = { _bl.x(), _tr.x() };
