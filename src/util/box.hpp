...
public:
  ...
  Box(T l = 0, T b = 0, T r = 0, T t = 0) : _bl(l, b), _tr(r, t) {
    // assert(l <= r && b <= t);
  }
  ...
  void setBounds(T l, T b, T r, T t) {
    setXL(l);
    setYL(b);
    setXH(r);
    setYH(t);
  ...
  void
  rotate90(const T x, const T y,
           const bool bClockWise); // rotate 90 degree with respect to (x, y)
  void rotate180(const T x,
                 const T y); // rotate 180 degree with respect to (x, y)
  ...
  void multi_diff(const std::list<const Box *> &vBox,
                  std::vector<Box> &result) const;
  void multi_diff(const std::list<std::uint32_t> &ord,
                  const std::vector<Box> &vBox, std::vector<Box> &result) const;
  ...
  static void intersection2(const Box &box1, const Box &box2,
                            std::vector<Box> &result);
  static void difference2(const Box &box1, const Box &box2,
                          std::vector<Box> &result);
  ...
  // operator
  bool operator<(const Box<T> &box) const { return bContain(box, *this); }
  bool operator<=(const Box<T> &box) const { return bCover(box, *this); }
  bool operator>(const Box<T> &box) const { return bContain(*this, box); }
  bool operator>=(const Box<T> &box) const { return bCover(*this, box); }

  bool operator==(const Box<T> &box) const {
    return _bl.x() == box.xl() && _bl.y() == box.yl() && _tr.x() == box.xh() &&
           _tr.y() == box.yh();
  }

  bool operator!=(const Box<T> &box) const { return !(*this == box); }

  friend std::ostream &operator<<(std::ostream &os, const Box &r) {
    os << '(' << r._bl.x() << ' ' << r._bl.y() << ' ' << r._tr.x() << ' '
       << r._tr.y() << ')';
    return os;
  }

  /// @brief Return the offset box
  Box<T> offsetBox(const Point<T> &origin) const {
    return Box<T>(xLo() + origin.x(), yLo() + origin.y(), xHi() + origin.x(),
                  yHi() + origin.y());
  }
  void offsetBy(const Point<T> &origin) {
    setXLo(xLo() + origin.x());
    setYLo(yLo() + origin.y());
    setXHi(xHi() + origin.x());
    setYHi(yHi() + origin.y());
  ...
  void printBoxInfo() const {
    std::cout << "  Bound : " << '(' << _bl.x() << ' ' << _bl.y() << ' '
              << _tr.x() << ' ' << _tr.y() << ')';
    std::cout << "  CenterXY : " << ' ' << (_bl.x() + _tr.x()) / 2 << ' '
              << (_bl.y() + _tr.y()) / 2 << ')' << std::endl;
  }
...
template <typename T> void Box<T>::shiftX(const T x) {
  setXL(_bl.x() + x);
  setXH(_tr.x() + x);
}

template <typename T> void Box<T>::shiftY(const T y) {
  setYL(_bl.y() + y);
  setYH(_tr.y() + y);
}

template <typename T> void Box<T>::shift(const T x, const T y) {
  shiftX(x);
  shiftY(y);
}
...
template <typename T> void Box<T>::rotate180(const T x, const T y) {
  Point<T> p1(_bl);
  Point<T> p2(_tr);
  p1.rotate180(x, y);
  p2.rotate180(x, y);
...
template <typename T> void Box<T>::flipX(const T x) {
  int l = _bl.x();
  int r = _tr.x();
  setXL(x + (x - r));
  setXH(x + (x - l));
...
template <typename T> void Box<T>::flipY(const T y) {
  int b = _bl.y();
  int t = _tr.y();
  setYL(y + (y - t));
  setYH(y + (y - b));
...
template <typename T> void Box<T>::expand(const T s) {
  setXL(_bl.x() - s);
  setYL(_bl.y() - s);
  setXH(_tr.x() + s);
  setYH(_tr.y() + s);
...
template <typename T> void Box<T>::expand(const T s, const int dim) {
  if (dim == 0) {
    setXL(_bl.x() - s);
    setXH(_tr.x() + s);
  } else {
...
template <typename T> void Box<T>::expandX(const T s) {
  setXL(_bl.x() - s);
  setXH(_tr.x() + s);
}

template <typename T> void Box<T>::expandY(const T s) {
  setYL(_bl.y() - s);
  setYH(_tr.y() + s);
}

template <typename T> void Box<T>::shrink(const T s) {
  setXL(_bl.x() + s);
  setYL(_bl.y() + s);
  setXH(_tr.x() - s);
  setYH(_tr.y() - s);
...
template <typename T> void Box<T>::shrinkX(const T s) {
  setXL(_bl.x() + s);
  setXH(_tr.x() - s);
}

template <typename T> void Box<T>::shrinkY(const T s) {
  setYL(_bl.y() + s);
  setYH(_tr.y() - s);
}
...
template <typename T>
void Box<T>::multi_diff(const std::vector<Box<T>> &vBox,
                        std::vector<Box<T>> &result) const {
  std::vector<T> hor = {_bl.y(), _tr.y()};
  std::vector<T> ver = {_bl.x(), _tr.x()};
  for (std::uint32_t i = 0; i < vBox.size(); ++i) {
    Box<T> &box = vBox[i];
  ...
  for (std::uint32_t i = 1; i < ver.size(); ++i)
    for (std::uint32_t j = 1; j < hor.size(); ++j)
      result.push_back(Box(ver[i - 1], hor[j - 1], ver[i], hor[j]));
}

template <typename T>
void Box<T>::multi_diff(const std::list<std::uint32_t> &ord,
                        const std::vector<Box<T>> &vBox,
                        std::vector<Box<T>> &result) const {
  std::vector<T> hor = {_bl.y(), _tr.y()};
  std::vector<T> ver = {_bl.x(), _tr.x()};
  for (std::uint32_t i : ord) {
...
template <typename T>
void Box<T>::multi_diff(const std::list<const Box<T> *> &vBox,
                        std::vector<Box<T>> &result) const {
  std::vector<T> hor = {_bl.y(), _tr.y()};
  std::vector<T> ver = {_bl.x(), _tr.x()};
...
bool Box<T>::bOverlap(const Box<T> &box1, const Box<T> &box2) {
  if (box1._bl.y() >= box2._tr.y())
    return false;
  if (box1._tr.y() <= box2._bl.y())
    return false;
  if (box1._bl.x() >= box2._tr.x())
    return false;
  if (box1._tr.x() <= box2._bl.x())
    return false;
  return true;
}
...
bool Box<T>::bOnBoundary(const Box &box, const Point<T> &pt, const int t) {
  switch (t) {
  case 0:
    return Box<T>::bOnBoundaryL(box, pt);
  case 1:
    return Box<T>::bOnBoundaryB(box, pt);
  case 2:
    return Box<T>::bOnBoundaryR(box, pt);
  case 3:
    return Box<T>::bOnBoundaryT(box, pt);
  default:
    assert(false);
  }
  return false;
...
template <typename T> bool Box<T>::bCover(const Box &box1, const Box &box2) {
  return bConnect(box1, box2.min_corner()) && bConnect(box1, box2.max_corner());
}

template <typename T> bool Box<T>::bContain(const Box &box1, const Box &box2) {
  return bInside(box1, box2.min_corner()) && bInside(box1, box2.max_corner());
}
...
template <typename T>
void Box<T>::intersection(const Box<T> &box1, const Box<T> &box2,
                          std::vector<Box<T>> &result) {
...
template <typename T>
void Box<T>::intersection2(const Box<T> &box1, const Box<T> &box2,
                           std::vector<Box<T>> &result) {
...
template <typename T>
void Box<T>::difference2(const Box<T> &box1, const Box<T> &box2,
                         std::vector<Box<T>> &result) {
...
template <typename T> inline void Box<T>::unionBox(const Box<T> &other) {
...
template <typename CoordType> struct tag<PROJECT_NAMESPACE::Box<CoordType>> {
...
struct indexed_access<PROJECT_NAMESPACE::Box<CoordType>, min_corner, 0> {
  ...
  static inline void set(PROJECT_NAMESPACE::Box<CoordType> &b,
                         CoordType const &value) {
    b.setXL(value);
  }
};
...
namespace polygon {
template <typename CoordType>
struct geometry_concept<PROJECT_NAMESPACE::Box<CoordType>> {
  typedef rectangle_concept type;
};

template <typename CoordType>
struct rectangle_traits<
    PROJECT_NAMESPACE::Box<CoordType>,
    typename gtl_same_type<
        typename PROJECT_NAMESPACE::Box<CoordType>::interval_type,
        typename PROJECT_NAMESPACE::Box<CoordType>::interval_type>::type> {
  typedef CoordType coordinate_type;
  typedef
      typename PROJECT_NAMESPACE::Box<CoordType>::interval_type interval_type;
  static inline interval_type
  get(const PROJECT_NAMESPACE::Box<CoordType> &rectangle,
      orientation_2d orient) {
    if (orient == HORIZONTAL) {
      return boost::polygon::interval_mutable_traits<interval_type>::construct(
          rectangle.xl(), rectangle.xh());
    } else {
      return boost::polygon::interval_mutable_traits<interval_type>::construct(
...
struct rectangle_mutable_traits<PROJECT_NAMESPACE::Box<CoordType>> {
  ...
  static inline void set(PROJECT_NAMESPACE::Box<CoordType> &rectangle,
                         orientation_2d orient, const T2 &interval) {
    if (orient == HORIZONTAL) {
  ...
  static inline PROJECT_NAMESPACE::Box<CoordType>
